 ...
 */
const getTrafficRulesConfig = (input, name) => {
    if (typeof input !== 'object' || input === null) {
        throw new FunctionBundlingUserError(`Could not parse ratelimit declaration of function '${name}'. Expecting an object, got ${input}`, {
            functionName: name,
            runtime: RUNTIME.JAVASCRIPT,
            bundler: NODE_BUNDLER.ESBUILD,
        });
    }
    const { windowSize, windowLimit, algorithm, aggregateBy, action } = input;
    if (typeof windowSize !== 'number' ||
        typeof windowLimit !== 'number' ||
        !Number.isInteger(windowSize) ||
        !Number.isInteger(windowLimit)) {
        throw new FunctionBundlingUserError(`Could not parse ratelimit declaration of function '${name}'. Expecting 'windowSize' and 'limitSize' integer properties, got ${input}`, {
            functionName: name,
            runtime: RUNTIME.JAVASCRIPT,
            bundler: NODE_BUNDLER.ESBUILD,
        });
    ...
    return {
        action: {
            type: action || RateLimitAction.Limit,
            config: {
                ...rewriteConfig,
                rateLimitConfig: {
                    windowLimit,
                    windowSize,
                    algorithm: algorithm || RateLimitAlgorithm.SlidingWindow,
                },
                aggregate: {
